...
public class MainActivity extends AppCompatActivity implements NoteFragment.
    ...
    public ArrayList<Note> retrieveNotes() {
        ...
        if (files != null) {
            for (File file : files) {
                ...
                String header = getPreferences(Context.MODE_PRIVATE).getString(file.getName(),
                        "No Heaader!");
    ...
    private void saveContent(Note note, String content) {
        ...
        Log.d("Saving tp Pref", "key = " + file.getName() + " value = " + note.
                getHeader());
    ...
    private String readContent(Note note) {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(new File(note.
                getFilePath())))) {
            String line;
